...
tags:
  - Rust
  - Async
excerpt: Use select! to wait on multiple branches, cancel slow paths, and build timeouts without blocking threads.
---
...
> Dropping futures can leak resources if a future holds a mutex guard across an await point—keep critical sections small and avoid `std::sync::Mutex` in async code; use `tokio::sync::Mutex`.

## Pinning and macro hygiene
