...
tags:
  - Web Platform
  - CSS
excerpt: Use document.startViewTransition to animate DOM updates with minimal JS, plus CSS view-transition-name for per-element choreography.
---
...
## Why pair with startTransition in React
...
## Name elements for paired motion
...
```css
/* Assign stable names to elements that should morph between states */
.card-title {
...
> Support is strong in Chromium; Safari and Firefox are catching up—keep the fallback branch (no `startViewTransition`) for older engines.
